...
class ObjectDetection:
        ...
        for ladder_box in ladder:
            for worker_box, worker_box_bounding, worker_class in worker:
                if (ladder_box[0] <= worker_box[0] <= ladder_box[2]) or (
                    ...
                    worker_height = round(
                        (worker_height_in_percentage / 100.0) * 2.0, 2
                    )
            ...
            for j, perBox2 in enumerate(person):
                if i != j:
                    if ((perBox1[1] + perBox1[3]) / 2) > perBox2[3] or (
                        ...
                        if (perBox1[0] - (perBox1[2] - perBox1[0]) / 2) < perBox2[
                            2
                        ] and (perBox1[2] + (perBox1[2] - perBox1[0]) / 2) > perBox2[0]:
    ...
    def draw_text_with_background(
        self, image, text, position, font_scale, color, thickness
    ):
    ...
    def detect_cutting_welding(self, image, results):
        ...
        for perBox in person:
            ...
            status_text = (
                "Worker with Hardhat" if hatDetected else "Worker without HardHat"
            )
            cv2.putText(
                image,
                status_text,
