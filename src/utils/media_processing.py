...
def _log_ffmpeg_output(stream, stream_id: str, output_type: str, process_type: str = "ffmpeg"):
    ...
    try:
        for line in iter(stream.readline, b''):
            if line.strip():
                ...
                if any(skip_pattern in decoded_line for skip_pattern in [
                ...
                if any(keyword in decoded_line.lower() for keyword in ["error", "failed", "invalid"]):
                    log_level = "error"
                    should_log = True
                elif any(keyword in decoded_line.lower() for keyword in ["warning", "deprecated"]):
                    log_level = "warning"
                    should_log = True
                elif decoded_line.startswith("ffmpeg version"):
                    log_level = "info"
                    should_log = True
                elif any(pattern in decoded_line for pattern in [
                    "Input #", "Output #", "Stream #", "Stream mapping:",
                    "Duration:", "encoder :", "video:", "audio:"
                ]):
                ...
                if should_log:
                    ...
                    extra_info = {"output_type": output_type, "process_type": process_type}
        ...
        log_event(
            ...
            stream_id=stream_id,
            extra={"output_type": output_type, "process_type": process_type, "error": str(e)}
        )
    finally:
        stream.close()
