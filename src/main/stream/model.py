...
class Stream:
    ...
    def create_stream(self):
        try:
            ...
            if result["status"] == "error":
                return self._create_error_response(
                    result["message"], result.get("error_code")
                )
    ...
    def restart(self):
        try:
            ...
            if result["status"] == "error":
                return self._create_error_response(
                    result["message"], result.get("error_code"), 500
                )
    ...
    def save_patrol_area(self):
        ...
        if not stream_id:
            return tools.JsonResp(
                {"status": "error", "message": "Missing stream_id in request data"}, 400
            )
        ...
        result = PatrolService.save_patrol_area(
            stream_id=stream_id, patrol_area=patrol_area
        )
    ...
    def save_patrol_pattern(self):
        ...
        result = PatrolService.save_patrol_pattern(
            stream_id=stream_id, patrol_pattern=patrol_pattern
        )
        status_code = 200 if result["status"] == "success" else 400
        if result["status"] == "error" and "not found" in result.get("message", ""):
            status_code = 404
    ...
    def preview_patrol_pattern(self):
        ...
        result = PatrolService.preview_patrol_pattern(
            stream_id=stream_id, patrol_pattern=patrol_pattern
        )
        status_code = 200 if result["status"] == "success" else 400
        return tools.JsonResp(result, status_code)
    ...
    def save_safe_area(self):
        ...
        if not stream_id:
            return tools.JsonResp(
                {"status": "error", "message": "Missing streamId in request data"}, 400
            )
        if not coords:
            return tools.JsonResp(
                {"status": "error", "message": "Missing coords in request data"}, 400
            )
    ...
    def start_patrol_if_enabled(stream_id, video_streaming, stream_doc):
        return PatrolService.start_patrol_if_enabled(
    ...
    def bulk_start_streams(self, stream_ids):
        ...
        if result["failed_streams"]:
            return tools.JsonResp(
                {
                    "message": f"Bulk start completed with {len(result['failed_streams'])} failures",
                    "data": result,
                },
                207,
            )
        return tools.JsonResp(
            {"message": "All streams started successfully", "data": result}, 200
        )
    ...
    def bulk_stop_streams(self, stream_ids):
        ...
        if result["failed_streams"]:
            return tools.JsonResp(
                {
                    "message": f"Bulk stop completed with {len(result['failed_streams'])} failures",
                    "data": result,
                },
                207,
            )
        return tools.JsonResp(
            {"message": "All streams stopped successfully", "data": result}, 200
        )
    ...
    def toggle_intrusion_detection(self):
        ...
        if result["status"] == "success":
            ...
            return self._create_error_response(
                result["message"], status_code=status_code
            )
    ...
    def toggle_patrol(self):
        try:
            ...
            if result["status"] == "success":
                ...
                if result.get("error_code") == "toggle_failed":
                    status_code = 500
                return self._create_error_response(
                    result["message"], status_code=status_code
                )
            ...
            return self._create_error_response(
                "Failed to toggle patrol", "toggle_failed", 500
            )
    ...
    def toggle_patrol_focus(self):
        ...
        stream_id = data.get("stream_id")
        if not stream_id:
            return self._create_error_response("Missing stream_id")
    ...
    def get_current_ptz_values(self):
        try:
            ...
            status_code = (
                404 if "not found" in result.get("message") else 400
            )  # Or 500 based on message
            return tools.JsonResp(result, status_code)
        except Exception:
    ...
    def ptz_controls(self):
        try:
            ...
            if not stream_id or not command:
                return self._create_error_response("Missing stream_id or command")

            result = PTZService.ptz_controls(stream_id, command, speed)
            if result["status"] == "success":
                return tools.JsonResp(result, 200)
            return tools.JsonResp(
                result, 400 if result.get("error_code") == "not_found" else 500
            )
        except Exception as e:
    ...
    def set_danger_zone(self):
        ...
        result = HazardService.set_danger_zone(
            stream_id=stream_id, coords=coords, image=image, static_mode=static_mode
        )
        status_code = 200 if result["status"] == "success" else 400
        if result["status"] == "error" and "not found" in result.get("message", ""):
            status_code = 404
    ...
    def set_camera_mode(self):
        ...
        if not stream_id:
            return tools.JsonResp(
                {"status": "error", "message": "Missing streamId"}, 400
            )

        result = HazardService.set_camera_mode(
            stream_id=stream_id, static_mode=static_mode
        )
        status_code = 200 if result["status"] == "success" else 400
        if result["status"] == "error" and "not found" in result.get("message", ""):
            status_code = 404
    ...
    def get_current_frame(self):
        try:
            ...
            if not stream_id:
                return tools.JsonResp(
                    {"status": "error", "message": "Missing stream_id"}, 400
                )
