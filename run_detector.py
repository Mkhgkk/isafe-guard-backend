...
#     VIDEO.show("PPE", "rtsp://admin:1q2w3e4r.@218.54.201.82:554/idis?trackid=2")

# run()
    

def main():
    # Initialize the VideoStreaming class
    video_streaming = VideoStreaming()

    # Specify the model to use and the RTSP link
    model_name = "PPE"
    rtsp_link = "rtsp://admin:1q2w3e4r.@218.54.201.82:554/idis?trackid=2"

    # Check if RTSP stream can be opened
    print(f"Attempting to open video stream from: {rtsp_link}")
    ...
    # Call the show method to start streaming and detection
    try:
        for frame_data in video_streaming.show(model_name, rtsp_link):
            # If you want to perform any additional processing or logging, do it here
            pass
